...
public:
    ...
    T second_minimum_key() const {
        Node *node {this->find_leftmost_node(this->root)};

        if (node->right != this->null) {
            return this->find_leftmost_node(node->right)->key;
        }
    ...
    T second_maximum_key() const {
        Node *node {this->find_rightmost_node(this->root)};

        if (node->left != this->null) {
            return this->find_rightmost_node(node->left)->key;
        }
...
public:
    ...
    Node *find_rightmost_node(Node *node) const {
        if (node->right == this->null) {
            return node;
        }

        return this->find_rightmost_node(node->right);
    }

    Node *find_leftmost_node(Node *node) const {
        if (node->left == this->null) {
            return node;
        }

        return this->find_leftmost_node(node->left);
    }
...
int main() {
    ...
    while (q--) {
        ...
        if (op == 1) {
            ...
            cout << rbt.minimum_key() << '\n';
        } else if (op == 6) {
            if (rbt.size() == 1 || rbt.empty()) {
                cout << "-\n";

                continue;
            }

            cout << rbt.second_minimum_key() << '\n';
        } else if (op == 7) {
            if (rbt.empty()) {
                cout << "-\n";
